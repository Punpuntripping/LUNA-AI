...
the upcoming migration) will call ``agents.deep_search_v4.orchestrator.run_full_loop``.
...
This file **imports from the future path** (``agents.deep_search_v4.orchestrator``).
...
-------------------
    ...
    python -m agents.deep_search_v4.cli --expander-prompt prompt_2 --aggregator-prompt prompt_1 "..."
    python -m agents.deep_search_v4.cli --no-compliance "..."
    python -m agents.deep_search_v4.cli --use-reranker "..."
...
# ---------------------------------------------------------------------------
from agents.deep_search_v4.orchestrator import FullLoopDeps, run_full_loop  # noqa: E402

# Load .env + configure logfire so the orchestrator's spans phone home
# (same wiring backend/app/main.py uses).
try:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Reg-search expander prompt variant (default: prompt_1).",
    )
    ...
    parser.add_argument(
        ...
        dest="reg_max_high",
        metavar="INT",
        help="Max high-relevance results per sub-query for reg reranker (default: 8).",
    )
    parser.add_argument(
        "--reg-max-medium",
        type=int,
        ...
        dest="reg_max_medium",
        metavar="INT",
        help="Max medium-relevance results per sub-query for reg reranker (default: 4).",
    )
    parser.add_argument(
        "--case-max-high",
        type=int,
        ...
        dest="case_max_high",
        metavar="INT",
        help="Max high-relevance results per sub-query for case reranker (default: 6).",
    )
    parser.add_argument(
        "--case-max-medium",
        type=int,
        default=4,
        dest="case_max_medium",
        metavar="INT",
        help="Max medium-relevance results per sub-query for case reranker (default: 4).",
    )
    parser.add_argument(
        "--compliance-max-high",
        type=int,
        default=6,
        dest="compliance_max_high",
        metavar="INT",
        help="Max high-relevance results for compliance reranker total pool (default: 6).",
    )
    parser.add_argument(
        "--compliance-max-medium",
        type=int,
        default=4,
        dest="compliance_max_medium",
        metavar="INT",
        help="Max medium-relevance results for compliance reranker total pool (default: 4).",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="enable_planner",
        help="Run the v4 planner before the parallel executors (drives invoke / focus / sectors).",
    )
    parser.add_argument(
        "--planner-model",
        default=None,
        dest="planner_model",
        help="Model registry key for the planner LLM (default: planner's own default).",
    )
...
def _build_deps(args: argparse.Namespace) -> FullLoopDeps:
    ...
    return FullLoopDeps(
        ...
        reg_max_high=args.reg_max_high,
        reg_max_medium=args.reg_max_medium,
        case_max_high=args.case_max_high,
        case_max_medium=args.case_max_medium,
        compliance_max_high=args.compliance_max_high,
        compliance_max_medium=args.compliance_max_medium,
