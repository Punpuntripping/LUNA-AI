"""Reranker-side unfolders for reg_search.

Both `precise` (compact) and `full` (article + siblings + references) modes
are consumed by the reranker depending on `state.unfold_mode`. Stage 2 (URA)
reuses these for now via `unfold_ura.py`.

Public surface (kept stable for `reranker.py` and `search.py`):
    - unfold_article / unfold_section / unfold_regulation                  (full mode)
    - unfold_article_precise / unfold_section_precise /
      unfold_regulation_precise                                            (precise mode)
    - unfold_article_with_siblings                                         (reranker tool)
    - format_unfolded_result / format_unfolded_result_precise              (formatters)
    - collect_references                                                   (post-pass)

Originally lived in `reg_search/unfold.py`; split out to mirror the
case_search / compliance_search stage-aware layout.
"""
