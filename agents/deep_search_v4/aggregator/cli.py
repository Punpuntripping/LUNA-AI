...
Usage:
    ...
    python -m agents.deep_search_v4.aggregator.cli --query-id 19

    # Replay a specific (query, timestamp) pair
    python -m agents.deep_search_v4.aggregator.cli --query-id 19 --timestamp 20260416_144941

    # Pick a different prompt variant
    python -m agents.deep_search_v4.aggregator.cli --query-id 19 --prompt prompt_2

    # Force the Draft-Critique-Rewrite chain
    python -m agents.deep_search_v4.aggregator.cli --query-id 19 --dcr

    # Override models (defaults: qwen3.6-plus primary, gemini-3-flash fallback)
    python -m agents.deep_search_v4.aggregator.cli --query-id 19 --primary qwen3.5-plus

    # Parse only (no LLM call) — useful for sanity checking
    python -m agents.deep_search_v4.aggregator.cli --query-id 19 --dry-run

    # Run against the golden set (5, 12, 14, 19, 27) with prompt_1
    python -m agents.deep_search_v4.aggregator.cli --golden

    # Run against every logged query
    python -m agents.deep_search_v4.aggregator.cli --all

    # Utilities
    python -m agents.deep_search_v4.aggregator.cli --list-prompts
    python -m agents.deep_search_v4.aggregator.cli --list-models
    python -m agents.deep_search_v4.aggregator.cli --list-logs
    python -m agents.deep_search_v4.aggregator.cli --read-log query_19/20260416_144941
