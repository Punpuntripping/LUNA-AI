...
def load_aggregator_input_from_ura(
    ...
    *,
    ...
    """Convert a URA 2.0 artifact into AggregatorInput.

    Thin wrapper over ``AggregatorInput.from_ura``. Kept here so existing
    replay / CLI code that imports from ``aggregator.log_parser`` keeps
    working without a rename churn. All the real reconstruction logic lives
    on the classmethod (single source of truth for URA -> AggregatorInput).
    """
    return AggregatorInput.from_ura(
        ura,
