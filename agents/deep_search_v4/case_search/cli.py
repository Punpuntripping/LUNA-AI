...
Usage:
    python -m agents.deep_search_v4.case_search.cli
    python -m agents.deep_search_v4.case_search.cli --query-id 23
    python -m agents.deep_search_v4.case_search.cli "your query"
    python -m agents.deep_search_v4.case_search.cli --query-id 23 --model or-gemma-4-31b
    python -m agents.deep_search_v4.case_search.cli --query-id 5 --expand-only
    python -m agents.deep_search_v4.case_search.cli --batch 1,5,9,14 --delay 5
    python -m agents.deep_search_v4.case_search.cli --list-prompts
    python -m agents.deep_search_v4.case_search.cli --list-models
    python -m agents.deep_search_v4.case_search.cli --list-logs
    python -m agents.deep_search_v4.case_search.cli --read-log query_23/20260405_140000
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            '  python -m agents.deep_search_v4.case_search.cli "سوابق قضائية في الفصل التعسفي"\n'
            '  python -m agents.deep_search_v4.case_search.cli --query-id 23 --model or-gemma-4-31b\n'
            "  python -m agents.deep_search_v4.case_search.cli --list-prompts\n"
            "  python -m agents.deep_search_v4.case_search.cli --list-models\n"
            "  python -m agents.deep_search_v4.case_search.cli --list-logs\n"
            "  python -m agents.deep_search_v4.case_search.cli --batch 1,5,9 --delay 5\n"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Planner-equivalent sector override. Comma- or pipe-separated sector "
            "names from the regulations vocab. Use 'none' to force no filter."
        ),
    )
    parser.add_argument(
        "--list-models",
...
async def main() -> None:
    ...
    # Channel filter still travels via deps (compose, not route). Sector
    # filter is now planner-shaped: pass through run_case_search's
    # ``sectors_override`` argument.
    if use_sectioned:
    ...
    result = await run_case_search(
        ...
        sectors_override=_parse_sector_override(args.sectors) if use_sectioned else None,
    )
