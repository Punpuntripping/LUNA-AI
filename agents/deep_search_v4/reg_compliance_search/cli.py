...
Usage:
    ...
    python -m agents.deep_search_v4.reg_compliance_search.cli

    # Full loop — specific query by ID
    python -m agents.deep_search_v4.reg_compliance_search.cli --query-id 23

    # Full loop — custom query text (auto-appended to test_queries.json)
    python -m agents.deep_search_v4.reg_compliance_search.cli "your query"

    # With explicit sector filter (planner-equivalent override)
    python -m agents.deep_search_v4.reg_compliance_search.cli --query-id 23 --sectors "العمل والتوظيف"

    # Expander + search only (no reranker)
    python -m agents.deep_search_v4.reg_compliance_search.cli --query-id 5 --expand-only

    # Aggregator only (re-use search results from a previous log)
    python -m agents.deep_search_v4.reg_compliance_search.cli --aggregate-only query_23/20260405_140000

    # Utilities
    python -m agents.deep_search_v4.reg_compliance_search.cli --list-prompts
    python -m agents.deep_search_v4.reg_compliance_search.cli --list-logs
    python -m agents.deep_search_v4.reg_compliance_search.cli --read-log query_23/20260405_140000
...
def _resolve_models(model_override: str | None = None) -> dict[str, str]:
    ...
    return {
        "expander": resolve("reg_compliance_expander", model_override),
        "reranker": resolve("reg_compliance_reranker", None),
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            '  python -m agents.deep_search_v4.reg_compliance_search.cli "أحكام إنهاء عقد العمل"\n'
            '  python -m agents.deep_search_v4.reg_compliance_search.cli "query" --sectors "العمل والتوظيف"\n'
            "  python -m agents.deep_search_v4.reg_compliance_search.cli --list-prompts\n"
            "  python -m agents.deep_search_v4.reg_compliance_search.cli --list-logs\n"
            "  python -m agents.deep_search_v4.reg_compliance_search.cli --read-log LOG_ID\n"
        ),
    )
...
async def run_batch(args) -> None:
    ...
    from .prompts import get_expander_prompt
    from .models import RegComplianceSearchDeps, RegSearchResult
    from .search import search_regulations_pipeline
    from .loop import _render_chunks_summary
    ...
    for run_idx, qid in enumerate(query_ids):
        ...
        for qi, (sq, sq_emb) in enumerate(zip(output.queries, sub_embeddings), 1):
            sq_deps = RegComplianceSearchDeps(
                supabase=supabase,
                embedding_fn=embed_regulation_query,
            )
...
async def main() -> None:
    ...
    if args.list_models:
        ...
        for slot in ("reg_compliance_expander", "reg_compliance_reranker"):
