"""Reranker for reg_search — single-pass keep-only LLM over mixed-type results.

Architecture: a classification-only LLM (no tools). Per sub-query,
``run_reranker_for_query`` :

1. Renders each search-result row as a labelled markdown block, dispatched on
   ``source_type``: regulation/appendix chunks render PRECISE (top rank band) or
   SIMPLE, while circulars and services render as flat one-hop blocks. The mode
   is decided by ``search.py`` and carried on each row as ``_mode``; the reranker
   does the actual unfold + render here so it owns the ``label -> row`` map.
...
# Source types that flow through the chunk unfold path (regulation body +
# appendix). Circular / service rows are flat and take their own block builders.
_CHUNK_SOURCE_TYPES = ("regulation", "appendix", "chunk")

# Content snippet cap for the forensic ``RerankedResult.content`` on flat
# (circular/service) rows — logging only; the URA stage re-fetches full content.
...
def create_reranker_agent(
    ...
    model = get_agent_model("reg_compliance_reranker", model_override)

    return Agent(
        model,
        name="reg_compliance_reranker",
...
#     "chunk":       {content row},         the fetched row (chunk / circular /
#                                           service) incl. merged RPC provenance
#     "source_type": "regulation"|"appendix"|"circular"|"service",
#     "mode":        "precise" | "simple" | "flat",
#     "rrf":         float,
#     "unfolded":    {dict from unfold_chunk_*} | None (None for flat rows),
#     "markdown":    str,                   rendered LLM-facing block
#   }
#
# The block builder dispatches on the row's ``source_type`` (set by search.py):
# chunk rows go through the two-shape unfold; circular / service rows are flat
# (the row IS the reranker view) and take their dedicated block formatters.
...
def _make_block(
    ...
    """Unfold + render one content row into a labelled reranker block (blocking).

    Dispatches on ``row["source_type"]``: circular/service rows render flat via
    their block builders (no DB hop); regulation/appendix rows go through the
    chunk unfold path (precise/simple per ``mode``).
    """
    rrf = float(rrf or 0.0)
    source_type = row.get("source_type", "regulation")
    ...
    if source_type == "circular":
        return {
            ...
            "rrf": rrf,
            "unfolded": None,
            "markdown": _format_circular_block(row, label, rrf),
        }
    if source_type == "service":
        return {
            ...
            "rrf": rrf,
            "unfolded": None,
            "markdown": _format_service_block(row, label, rrf),
        }

    # Regulation / appendix chunk → two-shape unfold.
    if mode == "precise":
        unfolded = unfold_chunk_precise(supabase, row)
    else:
        mode = "simple"
        unfolded = unfold_chunk_simple(supabase, row)
    unfolded["_score"] = rrf
    return {
        "label": label,
        "chunk": row,
...
def _assemble_markdown(blocks: list[dict[str, Any]]) -> str:
    ...
    parts: list[str] = [f"## نتائج البحث — {len(blocks)} نتيجة\n"]
    for b in blocks:
...
def _dropped_row(
    ...
    """Forensic descriptor for one dropped row (LLM drop or cap truncation).

    ``db_id`` is the bare content-row UUID (chunk / circular / service id); the
    adapter copies it straight into ``ref_id``. ``source_type`` carries the true
    topic type so Wave 2a's type-aware forensic routing can split by table.
    """
    row = block.get("chunk", {})
    unfolded = block.get("unfolded") or {}
    source_type = block.get("source_type", "chunk")
    title = (
        unfolded.get("title")
        or row.get("title")
        or row.get("service_name_ar")
        or row.get("intro_title")
        or ""
    )
    return {
        "db_id": row.get("id", "") or "",
...
def _assemble_result(block: dict[str, Any], decision: dict[str, Any]) -> RerankedResult:
    """Assemble a ``RerankedResult`` from a kept block + its keep dict.

    Dispatches on the block's ``source_type``. Regulation/appendix chunks keep
    the legacy ``source_type="chunk"`` shape (the existing ``reg_compliance_to_rqr`` path is
    unchanged; the regulation-vs-appendix distinction rides on ``source_row``).
    Circular/service rows carry their own ``source_type`` and the full fetched
    row in ``source_row`` for Wave 2a's type-aware adapter.
    """
    row = block["chunk"]
    source_type = block.get("source_type", "chunk")
    ...
    if source_type == "circular":
        return RerankedResult(
            ...
            content=(row.get("content") or "")[:_FLAT_CONTENT_SNIPPET],
            relevance=relevance,
            reasoning=reasoning,
            db_id=db_id,
            rrf=rrf,
    ...
    if source_type == "service":
        return RerankedResult(
            source_type="service",
            title=(row.get("service_name_ar") or row.get("intro_title") or "").strip(),
            content=(row.get("service_context") or "")[:_FLAT_CONTENT_SNIPPET],
            relevance=relevance,
            reasoning=reasoning,
            db_id=db_id,
            rrf=rrf,
    ...
    # Regulation / appendix chunk.
    unfolded = block.get("unfolded") or {}
    return RerankedResult(
        source_type="chunk",
        title=unfolded.get("title") or row.get("title", ""),
...
async def run_reranker_for_query(
    ...
    *,
    ...
    Args:
        ...
        chunks: Search-result content rows of any source type (chunk /
            circular / service). Each row carries ``source_type`` plus two
            routing keys: ``_mode`` (``"precise"`` | ``"simple"`` for chunks,
            ``"flat"`` for circular/service, set by search.py) and ``_rrf`` (the
            RPC similarity score). Chunk rows additionally carry the
            ``unfold_reranker.CHUNK_SELECT`` columns (+ ``corpus``).
    ...
    # -- Dedup kept blocks by generic source id (keep the higher-rrf copy) -----
    # ``chunk["id"]`` is the content-row UUID for every type (chunk / circular /
    # service), so this dedup is source-type-agnostic.
