"""Run logger for reg_compliance_search agent.
...
LOGS_DIR = (Path(_logs_override) / "v4_reg_compliance_search") if _logs_override else _default_logs
...
def save_run_overview_md(
    ...
    lines: list[str] = []
    lines.append(f"# reg_compliance_search — {log_id}")
    lines.append("")
    lines.append(f"| | |")
    lines.append(f"|---|---|")
