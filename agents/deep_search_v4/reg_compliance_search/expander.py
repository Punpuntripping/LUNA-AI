...
def get_expander_model_id() -> str:
    ...
    key = resolve_chain(AGENT_MODELS["reg_compliance_expander"])[0]
...
def create_expander_agent(
    ...
    Reasoning effort is fixed at medium by the ``reg_compliance_expander`` slot
    ...
    model = get_agent_model("reg_compliance_expander", model_override)

    return Agent(
        model,
        name="reg_compliance_expander",
