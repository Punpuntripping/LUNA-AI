...
Usage:
    python -m agents.deep_search_v3.aggregator.replay --all
    python -m agents.deep_search_v3.aggregator.replay --query 19
    python -m agents.deep_search_v3.aggregator.replay --query 19 --prompt prompt_2
    python -m agents.deep_search_v3.aggregator.replay --golden-only
    python -m agents.deep_search_v3.aggregator.replay --query 19 --dry-run
    python -m agents.deep_search_v3.aggregator.replay --limit 3
...
from agents.deep_search_v3.aggregator.log_parser import (
    discover_runs,
...
def _try_import_runner():
    try:
        from agents.deep_search_v3.aggregator import handle_aggregator_turn  # type: ignore
        from agents.deep_search_v3.aggregator import build_aggregator_deps  # type: ignore
    except ImportError:
        return None, None
...
async def _run_one(
    ...
    from agents.deep_search_v3.aggregator.logger import AggregatorLogger
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="agents.deep_search_v3.aggregator.replay",
