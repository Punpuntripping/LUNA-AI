...
Usage:
    ...
    python -m agents.deep_search_v3.reg_search.cli

    # Full loop — specific query by ID
    python -m agents.deep_search_v3.reg_search.cli --query-id 23

    # Full loop — custom query text (auto-appended to test_queries.json)
    python -m agents.deep_search_v3.reg_search.cli "your query"

    # With prompt selection
    python -m agents.deep_search_v3.reg_search.cli --query-id 23 --expander-prompt prompt_2

    # Expander + search only (no reranker)
    python -m agents.deep_search_v3.reg_search.cli --query-id 5 --expand-only

    # Aggregator only (re-use search results from a previous log)
    python -m agents.deep_search_v3.reg_search.cli --aggregate-only query_23/20260405_140000

    # Utilities
    python -m agents.deep_search_v3.reg_search.cli --list-prompts
    python -m agents.deep_search_v3.reg_search.cli --list-logs
    python -m agents.deep_search_v3.reg_search.cli --read-log query_23/20260405_140000
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            '  python -m agents.deep_search_v3.reg_search.cli "أحكام إنهاء عقد العمل"\n'
            '  python -m agents.deep_search_v3.reg_search.cli "query" --expander-prompt prompt_2\n'
            "  python -m agents.deep_search_v3.reg_search.cli --list-prompts\n"
            "  python -m agents.deep_search_v3.reg_search.cli --list-logs\n"
            "  python -m agents.deep_search_v3.reg_search.cli --read-log LOG_ID\n"
        ),
    )
...
async def run_expand_only(args, query: str, deps) -> None:
    ...
    # Sector filter from expander (prompt_2 only) — canonicalize to DB vocabulary
    from .sector_vocab import canonicalize_sectors
    raw_sectors = output.sectors or []
    if raw_sectors:
        filter_sectors = canonicalize_sectors(raw_sectors) or None
        if filter_sectors != raw_sectors:
            print(f"[expand-only] Sector filter (raw):  {' | '.join(raw_sectors)}")
            print(f"[expand-only] Sector filter (canon): {' | '.join(filter_sectors) if filter_sectors else '(all dropped — no filter)'}")
        else:
            print(f"[expand-only] Sector filter: {' | '.join(filter_sectors)}")
    else:
        filter_sectors = None
        print(f"[expand-only] Sector filter: (none)")
...
async def run_batch(args) -> None:
    ...
    for run_idx, qid in enumerate(query_ids):
        ...
        try:
            ...
            row["exp_sectors"] = output.sectors
            ...
            sectors_str = " | ".join(output.sectors) if output.sectors else "(none)"
        ...
        # Canonicalize sectors to exact DB vocabulary before filtering
        from .sector_vocab import canonicalize_sectors
        raw_sectors = output.sectors or []
        if raw_sectors:
            filter_sectors = canonicalize_sectors(raw_sectors) or None
            if filter_sectors != raw_sectors:
                dropped = [s for s in raw_sectors if s not in (filter_sectors or [])]
                mapped = [(r, c) for r, c in zip(raw_sectors, filter_sectors or []) if r != c] if filter_sectors else []
                if mapped:
                    for r, c in mapped:
                        print(f"  [SECTOR] '{r}' → '{c}' (canonicalized)")
                if dropped:
                    for d in dropped:
                        if not filter_sectors or d not in filter_sectors:
                            print(f"  [SECTOR] WARN: '{d}' has no DB match — dropped from filter")
        else:
            filter_sectors = None
