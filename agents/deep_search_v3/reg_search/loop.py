...
class SearchNode(BaseNode[LoopState, RegSearchDeps, RegSearchResult]):
    ...
    async def run(
        ...
        # Sector filter: applies uniformly to ALL queries in the batch.
        # Canonicalize LLM output to exact DB vocabulary before filtering.
        raw_sectors = (
            state.expander_output.sectors
        ...
        if raw_sectors:
            ...
            if canonical != raw_sectors:
                logger.info(
                    "SearchNode: sector canonicalization: %s → %s",
                    raw_sectors, canonical,
                )
            filter_sectors: list[str] | None = canonical or None
        else:
            filter_sectors = None
    ...
    async def run(
        ...
        async def _process_one(qi: int, sr_log: dict) -> None:
            ...
            try:
                ...
                # Stash usage + decisions for JSON logging
