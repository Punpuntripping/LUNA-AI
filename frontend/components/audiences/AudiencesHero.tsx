...
import { AUDIENCES_PAGE, AUDIENCES_STATS, PRIMARY_CTA_HREF } from "./content";

/**
 * Hero for the «ريحان يستهدف مين؟» page. Mirrors the landing hero treatment
 * (brand glow + dotted texture + corpus stat strip) but reframes the headline
 * around audience breadth rather than the source-cited-report differentiator.
 */
export function AudiencesHero() {
  return (
    <section className="relative overflow-hidden border-b border-border/40 bg-gradient-to-b from-primary/[0.05] via-background to-background">
      ...
      <div className="relative mx-auto max-w-3xl px-4 pb-14 pt-16 text-center sm:pt-24">
        ...
        <h1 className="mt-6 text-balance text-4xl font-bold leading-[1.25] tracking-tight text-foreground sm:text-[3.25rem] sm:leading-[1.2]">
          {AUDIENCES_PAGE.titleLead}{" "}
          <span className="text-primary">{AUDIENCES_PAGE.titleEmphasis}</span>
        </h1>

        <p className="mx-auto mt-5 max-w-2xl text-pretty text-base leading-relaxed text-muted-foreground sm:text-lg">
          {AUDIENCES_PAGE.subtitle}
        </p>
        ...
        <div className="mt-9 flex items-center justify-center">
          <Link
            ...
            className={cn(
              ...
              "gap-2 text-base font-semibold shadow-sm transition-shadow hover:shadow-md",
            )}
          >
            ابدأ الآن
        ...
        {/* Corpus stat strip — the scale that makes the breadth real. */}
        <dl className="mx-auto mt-12 flex max-w-2xl flex-wrap items-center justify-center gap-x-8 gap-y-4 border-t border-border/60 pt-7">
          {AUDIENCES_STATS.map((s) => (
