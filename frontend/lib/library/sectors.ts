...
// `compliance` / `circulars`), NOT the مكتبتي content-type tokens
// (`regulation` / `judgment` / `service` / `circular`). A reader arriving from
// /regulations must recognise the segment; the shelf's tokens are an internal
// storage vocabulary and never appear in a URL. If a second copy of this table
// ever appears, the two WILL drift — every consumer imports from here.
//
...
export const LIBRARY_TYPE_META: Record<LibraryType, LibraryTypeMeta> = {
  ...
  compliance: {
    label: "الخدمات",
    longLabel: "الخدمات والإجراءات الحكومية",
    wingPath: "/compliance",
    description: "أدلّة الخدمات الحكومية — الشروط والمستندات وخطوات التنفيذ.",
