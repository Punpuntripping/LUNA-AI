// The 28 issuing bodies of the /compliance wing — «الجهة».
//
...
// Alphabetical would bury وزارة العدل (115 guides) in the middle of a tail of
...
/**
 * `[slug, provider_name]` for the 28 entities, in the Python module's insertion
 ...
 * over each row is that entity's live guide count as measured 2026-08-22 —
...
const ENTITIES = [
  ...
  // 1 — the nine one-guide entities ship too (D3): a browse grid that silently
...
/** The 28 slugs in browse (corpus-volume) order. Never re-sort. */
 ...
 * `RESERVED_SLUGS` there). The 28 entity slugs are additionally reserved against
...
/**
 * True when `value` is one of the 28.
 *
...
/**
 * Arabic label for an entity slug, or `null` when it is not one of the 28.
 *
...
/**
 * `provider_name` → slug, or `null` for a name outside the 28.
 *
...
/**
 * The 28 switcher tiles, counts attached where the API supplied them.
 *
 ...
 * still renders all 28 links when the counts call soft-fails, and (b) every href
